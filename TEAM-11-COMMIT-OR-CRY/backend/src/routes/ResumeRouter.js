...
// Single resume analysis endpoint for employee flow
router.post('/analyze-single-resume', VerifyToken, upload.single('resume'), async (req, res) => {
    try {
        const { jobDescription } = req.body
        const file = req.file
        ...
        console.log(`🎯 Analyzing single resume: ${file.originalname}`)

        // Process the single resume file
        const processedResumes = await processResumeFiles([file])
        
        const successfullyProcessed = processedResumes.filter(resume => !resume.error)
        if (successfullyProcessed.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'Failed to process resume file. Please check file format and content.',
                error: processedResumes[0]?.error
            })
        }

        // Use the same OpenAI analysis as batch processing for consistency
        let analysisResult
        let analysisMethod = 'ai'
        ...
        try {
            ...
            // Use the same OpenAI service for consistent scoring
            const rankedResults = await rankResumesWithOpenAI(jobDescription, successfullyProcessed)
            analysisResult = rankedResults[0] // Get the first (and only) result

            console.log(`✅ Single resume analysis completed successfully`)

        } catch (aiError) {
            console.error('🚨 OpenAI analysis failed for single resume, using fallback:', aiError.message)
            analysisMethod = 'fallback'
            const fallbackResults = getFallbackRanking(successfullyProcessed, jobDescription)
            analysisResult = fallbackResults[0]
        }

        // Transform the result to match employee dashboard expectations
        const employeeResult = {
            overallScore: analysisResult.score,
            matchedSkills: analysisResult.keyMatches || [],
            missingSkills: analysisResult.missingSkills || [],
            explanation: analysisResult.explanation || 'Analysis completed successfully',
            strengths: extractStrengths(analysisResult.explanation),
            improvements: extractImprovements(analysisResult.explanation),
            categories: analysisResult.scoreBreakdown || {
                experience: Math.max(60, Math.min(95, analysisResult.score - 3)),
                skills: Math.max(60, Math.min(95, analysisResult.score + 2)),
                education: Math.max(60, Math.min(95, analysisResult.score - 1)),
                keywords: Math.max(60, Math.min(95, analysisResult.score))
            },
            analysisMethod: analysisMethod
        }
        ...
        res.status(200).json({
            success: true,
            message: `Resume analysis completed using ${analysisMethod === 'ai' ? 'OpenAI' : 'fallback'} analysis`,
            data: employeeResult
        })

    } catch (error) {
        console.error('🚨 Error in analyze-single-resume endpoint:', error)
...
// Helper functions for extracting insights from AI explanation
function extractStrengths(explanation) {
    const strengthKeywords = ['strong', 'excellent', 'good', 'relevant', 'experienced', 'skilled']
    ...
    if (strengths.length === 0) {
        return ['Strong technical background', 'Relevant experience with job requirements']
    }
    
    return strengths.map(s => s.trim())
}

function extractImprovements(explanation) {
    const improvementKeywords = ['missing', 'lack', 'need', 'should', 'could', 'improve']
    const sentences = explanation.split(/[.!?]+/).filter(s => s.trim().length > 10)
    
    const improvements = sentences.filter(sentence => 
    ...
    return improvements.map(s => s.trim())
}

// Debug endpoint to test scoring consistency
router.post('/debug-scoring-consistency', VerifyToken, upload.single('resume'), async (req, res) => {
    try {
        const { jobDescription, testCount = 3 } = req.body
        const file = req.file
        ...
        console.log(`🧪 Testing scoring consistency with ${testCount} runs...`)
        ...
        // Run multiple analysis with same input
        for (let i = 0; i < testCount; i++) {
            try {
                ...
                scores.push({
                    ...
                    explanation: rankedResults[0].explanation,
                    timestamp: new Date().toISOString()
                })
                console.log(`📊 Run ${i + 1}: Score = ${rankedResults[0].score}`)
            } catch (error) {
                scores.push({
                    run: i + 1,
                    error: error.message,
        ...
        const validScores = scores.filter(s => !s.error).map(s => s.score)
        ...
        res.status(200).json({
            success: true,
            message: `Scoring consistency test completed`,
            data: {
                resumeName: file.originalname,
                testRuns: testCount,
                successfulRuns: validScores.length,
                scores: scores,
                consistency: {
                    ...
                    isConsistent: scoreVariance <= 5, // Within 5 points is considered consistent
                    consistencyRating: scoreVariance <= 2 ? 'Excellent' : 
                                     scoreVariance <= 5 ? 'Good' : 
                                     scoreVariance <= 10 ? 'Fair' : 'Poor'
                }
            }
        })

    } catch (error) {
        console.error('🚨 Error in debug-scoring-consistency:', error)
        res.status(500).json({
            success: false,
